...
sub availability {
  ...
  if (!$self->{'_availability'}) {
    ...
    $availability->{'not_strain'} = $self->species_defs->parent_strain ? 0 : 1; #availability to check if species is a strain or not, it has to be this way round (used in Gene Configuration to disable main compara view on strain species)
...
sub get_homologue_alignments {
  ...
  my $strain_tree;
  if ($hub->param('data_action') =~ /strain_/i) {
    $strain_tree = $clustersets->{$hub->species_defs->get_config($hub->species,'SPECIES_PRODUCTION_NAME')};
  }
  my $msa;
